...
<template>
  <section class="question-screen">
    <div class="question-header">
      <small>Question {{ questionNumber }} / {{ totalQuestions }}</small>
      <div class="progress">
    ...
    <div class="icon-circle">
      <ScrollText :size="64" stroke-width="1.6" />
    </div>

    <AppCard class="question-card">
      <h1>{{ question.text }}</h1>

      <div v-if="question.type === 'multiple-choice'" class="answers">
        <button
...
.icon-circle {
  ...
  border-radius: 50%;
  color: var(--gold);
  box-shadow: 0 0 24px rgba(214, 179, 106, 0.14);
}
